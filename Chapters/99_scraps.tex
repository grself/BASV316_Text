...
% Add ``What to expect'' and ``Summary'' sections (Ch 1 has an ``objectives'' box and ``summary'' section already)
% Add example research reports

% Maybe start each chapter with a photo and ``Introduction'' paragraph, then the Objectives, then the text, finally, a ``Summary''. Maybe use the Text Box object for the objectives and summary section.
...
% Mixed Methods
% 14 Mixed: Rough draft: cleaned up language

% Reporting
% 15 Presenting: Rough Draft: cleaned up language. I can expand this greatly with ``how to'' instructions for both oral and written presentations. Maybe add a section on dissertations? In my reference section, see ``communication-for-business-success'' for a lot of information.

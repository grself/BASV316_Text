...
\begin{center}
	\begin{objbox}{Objectives}
		\begin{itemize}
			...
			\item Define database and find several public databases that may be useful for research projects
...
\begin{description}
	...
	\item[\Gls{semanticdiffscale}] This is a multi-item scale where respondents are asked to indicate their opinions or feelings toward a single statement using different pairs of adjectives framed as polar opposites. For instance, the construct ``attitude toward health insurance'' can be measured using three items shown in Table \ref{tab06.03}. As in the Likert scale, the overall scale score may be a summation of individual item scores. Notice that in Likert scales, the statement changes but the anchors remain the same across items. However, in semantic differential scales, the statement remains constant, while the anchors (adjective pairs) change across items. Semantic differential is believed to be an excellent technique for measuring people's attitude or feelings toward objects, events, or behaviors. 
...
Following are four examples of excess kurtosis. Notice that as the excess kurtosis increases the tails become longer. 
...
\begin{table}[H]
	...
	{\small
		...
		\begin{longtable}{
			...
			\rowcolor{captionwhite}
			\caption{Tests of Association Between Two Samples}
			\label{tab06.06}
		\end{longtable}
	} % End small
\end{table}
...
\begin{table}[H]
	...
	{\small
		...
		\begin{longtable}{
			...
			\textbf{Party} & \textbf{Clinton} & \textbf{Trump} & \textbf{Other} \endhead
			\hline
			Democrats & $ 89\% $ & $ 8\% $ & $ 3\% $ \\
			Republicans & $ 8\% $ & $ 88\% $ & $ 4\% $ \\
...
\begin{table}[H]
	...
	{\small
		...
		\begin{longtable}{
			...
			\textbf{} & \textbf{mpg} & \textbf{disp} & \textbf{hp} \endhead
			\hline
			\textbf{mpg}  & $ +1.00 $ & $ -0.85 $ & $ -0.78 $ \\
			\textbf{disp} & $ -0.85 $ & $ +1.00 $ & $ +0.79 $ \\
			\textbf{hp}   & $ -0.78 $ & $ +0.79 $ & $ +1.00 $ \\
			\rowcolor{captionwhite}
...
\section{Key Takeaways}\label{ch06:summary}
...
\begin{center}
	\begin{tkawybox}{Data}
		\begin{itemize}
			...
			\item Data has three commonly-reported properties: distribution, excess kurtosis, and skew
			\item Database are collections of data and many public databases can be useful for research projects
			\item Many types of statistical tests are used for central measures, spread, frequency, correlation, parametric hypothesis testing, and nonparametric hypothesis testing.
			\item Data mining is the process of extracting intelligence from huge data sets and the three most common forms of data mining are clustering, decision trees, and market basket.
		\end{itemize}
	\end{tkawybox}
\end{center}
